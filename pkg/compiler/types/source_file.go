...
func (lid LanguageId) CastFile(f klotho_io.File) (*SourceFile, bool) {
...
func (f *SourceFile) Reparse(newProgram []byte) (err error) {
	...
	if err == nil {
		...
		err = klotho_errors.WrapErrf(err, "could not reparse %s", f.Path())
	}
	return
}
...
func (f *SourceFile) Clone() klotho_io.File {
...
func NewSourceFile(path string, content io.Reader, language SourceLanguage) (f *SourceFile, err error) {
	...
	if err != nil {
		err = klotho_errors.WrapErrf(err, "error reading from %s", path)
		return
	}
