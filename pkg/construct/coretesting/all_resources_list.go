...
type (
	Methods struct {
		...
		//
		//	BaseConstructRefs func() []github.com/klothoplatform/klotho/pkg/types.AnnotationKey
...
// [assert.Assertions].
func FindAllResources(a *assert.Assertions, from []construct.Resource) []TypeRef {
	...
	// Find the methods for construct.Resource
	var t2 reflect.Type = reflect.TypeOf((*construct.Resource)(nil)).Elem()
	...
	// Within those packages, find all structs that implement construct.Resource

	// allTypesRef is basically a cache: for each TypeRef we see, is it a construct.Resource?
	...
	for pkg := range packageNames {
		...
		for ref, methods := range resourcesTypes {
			...
			// Ignore all interfaces, and all structs/typedefs that don't implement construct.Resource
