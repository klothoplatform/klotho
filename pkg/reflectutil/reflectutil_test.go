...
func TestGetField(t *testing.T) {
	...
	exampleMapWithPeriods := map[string]any{
		"A.B": map[string]any{
			"C.D": "value",
		},
		"E": map[string]any{
	...
	tests := []struct {
		...
		{
			name:      "Map access with period in key",
			v:         exampleMapWithPeriods,
			fieldExpr: `[A.B][C.D]`,
			want:      "value",
		},
		{
			name:      "Nested map access with period in key",
			v:         exampleMapWithPeriods,
			fieldExpr: `E[F.G][0][H.I]`,
			want:      "nested value",
		},
		{
		...
		{
			name:      "Bracket notation for non-map",
			v:         example,
			fieldExpr: `A[B]`,
			wantErr:   true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				assert.Errorf(err, "GetField() wantErr %v", tt.wantErr)
				return
			} else {
				assert.EqualValues(tt.want, GetConcreteValue(got))
			}
		})
	}
}
