...
func ConfigureField(resource core.Resource, fieldName string, value interface{}, zeroValueAllowed bool, graph *core.ResourceGraph) error {
	field, setMapKey, err := parseFieldName(resource, fieldName)
	if err != nil {
		return err
	}
...
func configureField(val interface{}, field reflect.Value, dag *core.ResourceGraph, zeroValueAllowed bool) error {
	...
	fmt.Println("configuring", field, "with", val)
		...
		for _, key := range reflect.ValueOf(val).MapKeys() {
			for i := 0; i < field.NumField(); i++ {
				if field.Type().Field(i).Name == key.String() {
					fmt.Println("configuringgg", field.Type().Field(i).Name, "with", reflect.ValueOf(val).MapIndex(key).Interface())
...
func getFieldFromIdString(id string, dag *core.ResourceGraph) any {
	...
	field, _, err := parseFieldName(res, arr[1])
	if err != nil {
		return nil
	}
...
func parseFieldName(resource core.Resource, fieldName string) (reflect.Value, *SetMapKey, error) {
		...
		if key != "" {
			if field.Kind() == reflect.Map {
				...
				if field.MapIndex(reflect.ValueOf(key)).IsValid() {
					for i := 0; i < field.MapIndex(reflect.ValueOf(key)).NumField(); i++ {
						newField.Field(i).Set(field.MapIndex(reflect.ValueOf(key)).Field(i))
					}
				}
